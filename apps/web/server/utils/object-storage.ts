...
export function getCachedObjectStorageAdapter(bucket: string): ObjectStorageAdapter {
  ...
  const cacheKey = [
    process.env.OBJECT_STORAGE_DRIVER?.trim().toLowerCase() || 'gcs',
    process.env.LOCAL_OBJECT_STORAGE_ROOT?.trim() || '',
    normalizedBucket
  ].join(':');
  const existingAdapter = globalObject.__pepetexObjectStorageAdapters.get(cacheKey);
  ...
  globalObject.__pepetexObjectStorageAdapters.set(cacheKey, adapter);
