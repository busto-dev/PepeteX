...
import { exportPdfQueueName, type ExportPdfJobPayload } from '@pepetex/queue';
...
export default defineEventHandler(async (event) => {
  ...
  const queue = new Queue<ExportPdfJobPayload>(exportPdfQueueName, { connection: redis });

  const payload: ExportPdfJobPayload = {
    jobId,
  ...
  try {
    await queue.add('export-pdf', payload, {
      jobId,
      attempts: 2,
