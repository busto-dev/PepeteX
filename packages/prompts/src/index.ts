...
export const pepeteXAgentToolContract = [
  ...
  '- request_approval: supervisor-only tool used before destructive or broad risky changes such as deleting slides, replacing multiple existing slides, or overwriting a non-empty deck.',
  '- write_slide: supervisor mutation tool for AGENT_COMMAND slide creation or replacement, one slide per tool call. Payload shape is { "operation": "insert|replace", "position": "end|start|before|after|index", "referenceSlideId": "<optional>", "index": 0, "slide": { "id": "...", "title": "...", "html": "...", "css": "...", "assets": [], "charts": [], "diagrams": [] }, "summary": "..." }.',
  '- patch_slide: supervisor mutation tool for selected-slide regeneration, submitted comments, submitted tweaks, reorder/delete, selected edits, and other bounded edits. Payload shape is { "patch": { "operations": [ ...PepeteX patch operations... ] }, "summary": "..." }. For submitted comments, include commentIds and cover every submitted comment before finish_generation.',
  ...
  '- finish_generation: supervisor-only commit tool. Call only after validation passes. Payload shape is { "summary": "<concise summary of actual committed changes>" }.',
...
const pepeteXSecurityRules = [
  ...
  'PPTX export fidelity rules (the slide must look the same in HTML preview and the exported .pptx — dom-to-pptx is the converter and is strict about geometry). The following CSS is FORBIDDEN and will be REJECTED by the validator — using any of these will cause your output to fail and force a repair pass:',
  '- transform other than rotate(...). Do NOT use transform: translate, scale, skew, matrix, or perspective; replace them with explicit left/top/width/height/font-size declarations. Non-rotate transforms silently rasterize via html2canvas and lose precise positioning. transform: rotate(...) remains allowed.',
  '- mix-blend-mode and backdrop-filter: forbidden. dom-to-pptx silently drops these properties, so the preview shows mixed/blurred colors but the export shows fully opaque shapes. Instead, layer solid shapes with rgba() fills (typically rgba(R,G,B,0.30–0.55)). Simple alpha compositing is reproducible in PPTX and looks the same in preview and export.',
  '- clip-path, mask, and filter: forbidden. dom-to-pptx either drops them or rasterizes the region to a low-fidelity PNG via html2canvas. Achieve the visual with solid shapes, border-radius, box-shadow, or rgba() layered fills. For decorative cropping, position the asset at the canvas edge and let .pepetex-slide{overflow:hidden} clip it.',
  '- radial-gradient(...) and conic-gradient(...): forbidden. dom-to-pptx only supports linear-gradient(...) natively. Use linear-gradient with multiple stops to approximate radial/conic looks.',
  ...
  '- Inner stacking: when a child needs to sit at a specific spot inside an inner container, use position:absolute with explicit left/top relative to that container, not transform:translate. The container must have position:relative.',
